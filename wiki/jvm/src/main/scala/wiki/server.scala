...
object Server extends SimpleRoutingApp with Api {
  val serverDate = "December-14-10am-2015"
                ...
                onSuccess(valid) {
                  case Some(user: String) => {
                    if ((user == "rik") || (user == "will")) {
                      println("env" !!)
                      println(Seq("git", "-C", Paths.wikiBase, "pull", "origin", "master") !!)
                      println(Seq("git", "-C", Paths.tripsXMLBase, "pull", "origin", "master") !!)
            ...
            path("auth") {
              ...
              optionalCookie("token") {
                case Some(token) => {
                  ...
                  onSuccess(valid) {
                    case Some(user: String) => {
                      complete { upickle.default.write(Some(token.content)) }
                    }
                    case None =>
  ...
  var comments: Map[String, Set[Comment]] = Map().withDefault((s: String) => {
    Set[Comment](
